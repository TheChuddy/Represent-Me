...
struct _V_UserInfoSheetView: View {
    ...
    var body: some View {
        NavigationView {
            List {
                ...
                Section {
                    HStack {
                        Text("Phone Number:").padding(.trailing)
                        TextField("Enter your number", text: $phoneEntry).multilineTextAlignment(.trailing)
                    }
                }
            }
    ...
    func saveButton() {
        self.homeLibrary.updateUserInfo(name: self.nameEntry, address: self.streetAddressEntry, city: self.cityEntry, state: self.region, zip: self.zipCodeEntry, num: self.phoneEntry)
