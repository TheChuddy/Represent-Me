...
class TemplateLibrary: ObservableObject {
    ...
    @Published private var user = UserInformation(userName: "", userStreetAddress: "", userCity: "", userState: StatesEnum.Alaska, userZipCode: "", userPhoneNum: "", sortingByPopular: true, sortingByNew: false, sortingByRegion: false, sortRegion: StatesEnum.National)
    ...
    func getUserPhone() -> String {
        self.user.getUserPhone()
    }
    
    func updateUserInfo(name: String, address: String, city: String, state: StatesEnum, zip: String, num: String) {
        self.user.updateUserInfo(name: name, address: address, city: city, state: state, zip: zip, num: num)
    }
